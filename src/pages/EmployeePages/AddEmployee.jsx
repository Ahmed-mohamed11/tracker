...
export default function AddEmployeeForm({ handleClose, handleAddEmployee }) {
  ...
  const isAllSelected = () => {
    const allItems = options.flatMap((group) =>
  ...
  const handleSelectAll = () => {
    ...
    setIsOpen(false); // إغلاق القائمة بعد اختيار الكل
  };
                        ...
                        <div className="pr-4 pb-2">
                          <label className="flex items-center px-4 py-2 hover:bg-gray-100 cursor-pointer transition-colors duration-150">
                            <input
                              ...
                              onChange={() =>
                                handleItemClick("تحديد الكل", group.name)
                              }
                              className="w-4 h-4 text-blue-600 border-gray-300 rounded focus:ring-blue-500"
                            />
                            <span className="mr-3 text-sm font-medium text-gray-700">
                          ...
                          {group.items.map((item) => (
                            ...
                            >
                              <input
                                ...
                                onChange={() =>
                                  handleItemClick(item.id, group.name)
                                }
                                className="w-4 h-4 text-blue-600 border-gray-300 rounded focus:ring-blue-500"
                              />
                              <span className="mr-3 text-sm font-medium text-gray-700">
