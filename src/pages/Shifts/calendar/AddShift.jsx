...
export default function ShiftForm({
  ...
  const validateTimes = () => {
    ...
    if (checkInTime && maxCheckInTime) {
      if (maxCheckInTime <= checkInTime) {
        newErrors.maxCheckInTime =
          "أقصى وقت لتسجيل الدخول يجب أن يكون أكبر من وقت تسجيل الدخول";
      }
    }

    if (checkInTime && shiftEndTime) {
      if (shiftEndTime <= checkInTime) {
        newErrors.shiftEndTime =
          "وقت الانصراف يجب أن يكون أكبر من وقت تسجيل الدخول";
      }
    }

    if (maxCheckInTime && shiftEndTime) {
      if (shiftEndTime <= maxCheckInTime) {
        newErrors.shiftEndTime =
          "وقت الانصراف يجب أن يكون أكبر من أقصى وقت لتسجيل الدخول";
      }
    }
  ...
  const handleSubmit = async () => {
    if (!validateTimes()) {
      toast.error("تحقق من صحة الأوقات قبل الحفظ");
      return;
    }
    ...
    const payloadBase = {
      ...
      ...(isChecked
        ? {}
        : {
            // Include only if not a vacation day
            ...
            flexible_minutes: maxCheckInTime
              ? Math.max(
                  0,
                  (new Date(`1970-01-01T${maxCheckInTime}:00`) -
