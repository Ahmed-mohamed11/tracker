...
const ReportsTable = ({ openCreate }) => {
  ...
  useEffect(() => {
    ...
    lastWeek.setDate(today.getDate() - 7); // Set to one week before today
  ...
  const fetchData = useCallback(async () => {
    try {
      ...
      // تصفية الأنشطة التي من النوع attendance
      const attendanceData = registrationRequests.filter(
        (request) => request.activity_type === "attendance"
      );
      ...
      const formattedData = attendanceData.map((request) => ({
