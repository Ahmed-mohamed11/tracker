...
export default function Dashboard() {
    ...
    const fetchData = useCallback(async () => {
        try {
            ...
            const [branchesResponse, entitiesResponse, employeesResponse] = await Promise.all([
                axios.get('https://bio.skyrsys.com/api/branch/branches/', { headers }),
    ...
    const handleDataDisplay = useCallback(async () => {
        ...
        try {
            const response = await axios.get(`https://bio.skyrsys.com/api/dashboard/?${params.toString()}`, {
...
console.log('55', response.data);
            ...
            setChartData2({
                second_chart: {
                    attendance_before_time_counts: second_chart.attendance_before_time_counts || 0,
                    departure_before_time_counts: second_chart.departure_before_time_counts || 0,
                    overtime_counts: second_chart.overtime_counts || 0,
                },
            });
    ...
    useEffect(() => {
        if (selectedBranch && selectedEntity && selectedEmployee && startDate && endDate) {
            handleDataDisplay();
        }
    }, [handleDataDisplay]);

    const isFormValid = useMemo(() => {
        return selectedBranch && selectedEntity && selectedEmployee && startDate && endDate;
    }, [selectedBranch, selectedEntity, selectedEmployee, startDate, endDate]);
